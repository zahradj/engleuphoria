  ...
  try {
    ...
    if (emailError) {
      ...
      await adminClient.from('system_emails').insert({
        ...
        error_message: emailError.message || 'send-transactional-email invocation failed',
        related_entity_id: interview.id,
        related_entity_type: 'teacher_interview',
        metadata: { interviewDate, interviewTime, interviewLink: interview.zoom_link },
      })
    ...
    await adminClient.from('system_emails').insert({
      ...
      related_entity_id: interview.id,
      related_entity_type: 'teacher_interview',
      metadata: { interviewDate, interviewTime, interviewLink: interview.zoom_link, confirmUrl },
    })

    console.log(`✅ Interview invite sent to ${app.email} for interview ${interview.id}`)
