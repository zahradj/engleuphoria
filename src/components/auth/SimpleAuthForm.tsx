              ...
              <CardContent className="px-8 pb-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {mode === 'signup' && <div className="space-y-2">
                      ...
                      <div className="relative group">
                        <Lock className="absolute left-4 top-1/2 transform -translate-y-1/2 h-5 w-5 text-slate-600 group-focus-within:text-slate-800 transition-colors" />
                        <Input id="confirmPassword" type={showConfirmPassword ? "text" : "password"} value={formData.confirmPassword} onChange={e => handleInputChange('confirmPassword', e.target.value)} placeholder="Confirm your password" disabled={loading} required className="h-14 pl-12 pr-12 bg-[#dfc7cc] border-purple-200/60 text-slate-700 placeholder:text-slate-500 focus:bg-[#dfc7cc]/90 focus:border-purple-400 focus:ring-2 focus:ring-purple-200 transition-all duration-300 rounded-xl shadow-sm hover:shadow-md" />
                        <Button type="button" variant="ghost" size="sm" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 h-10 w-10 p-0 text-slate-500 hover:text-purple-600 hover:bg-purple-100/50 rounded-lg transition-all duration-200" disabled={loading}>
