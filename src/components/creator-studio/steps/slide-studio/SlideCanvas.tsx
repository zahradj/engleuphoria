...
const SlideMedia: React.FC<{ slide: PPPSlide }> = ({ slide }) => {
  ...
  // ── Skeleton loader: image is being auto-generated by the AI pipeline ──
  const hasPrompt = (slide.visual_keyword || '').trim().length > 0;
  if (hasPrompt && !slide.custom_image_url && !slide.custom_video_url && !slide.youtube_video_id) {
    return (
      <div className="mx-auto rounded-2xl overflow-hidden w-full max-w-sm h-48 sm:h-56 bg-gradient-to-br from-slate-100 via-slate-50 to-slate-100 dark:from-slate-800 dark:via-slate-900 dark:to-slate-800 flex flex-col items-center justify-center gap-2 animate-pulse">
        <span className="text-3xl">🎨</span>
        <p className="text-xs font-semibold text-slate-400 dark:text-slate-500">AI is painting this scene…</p>
      </div>
    );
  }
